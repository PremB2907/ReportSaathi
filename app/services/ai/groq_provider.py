...
class GroqProvider(BaseAIProvider):
    """
    Groq Cloud API provider supporting Vision and fast conversational models.
    """
    def __init__(self):
        self.api_key = os.environ.get("GROQ_API_KEY", "").strip()
        self.model = os.environ.get("GROQ_VISION_MODEL", "qwen/qwen3.6-27b").strip()
        self.api_url = "https://api.groq.com/openai/v1/chat/completions"
        self.models_url = "https://api.groq.com/openai/v1/models"
        self.chat_model = "llama3-8b-8192"
    ...
    def check_health(self) -> dict:
        ...
        try:
            ...
            if self.model not in active_ids:
                # If Qwen model isn't active, see if any fallback vision model exists
                vision_models = [m_id for m_id in active_ids if "vision" in m_id or "qwen" in m_id]
                if not vision_models:
                    ...
                    # Dynamically swap model to another supported vision model
        ...
        for start_idx in reversed(indices):
            ...
            if end_idx > start_idx:
                try:
                    ...
                    pass
        raise ValueError(f"Could not parse a valid structured JSON output from Groq raw response: {raw_content}")
